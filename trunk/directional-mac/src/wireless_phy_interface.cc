...
void WirelessPhyInterfaceSleepTimer::expire(Event *) {
	a_->updateSleepEnergy();
}

/* ======================================================================
 WirelessPhyInterface Interface
 ====================================================================== */
static class WirelessPhyInterfaceClass: public TclClass {
public:
	WirelessPhyInterfaceClass() : TclClass("Phy/WirelessPhyInterface") {}

	TclObject* create(int, const char* const *) {
		return (new WirelessPhyInterface);
	}
} class_WirelessPhyInterface;

WirelessPhyInterface::WirelessPhyInterface() :
	Phy(), sleep_timer_(this), status_(IDLE) {
	/*
...
int WirelessPhyInterface::command(int argc, const char* const * argv) {
	TclObject *obj;
...
void WirelessPhyInterface::sendDown(Packet *p) {
	/*
	 * Sanity Check
	 */
	assert(initialized());
	...
	if (em()) {
		//node is off here...
		if (isInterfaceOn() != true) {
			Packet::free(p);
			return;
		}
		if (isInterfaceOn() == true && isInterfaceOn() == true) {
	...
	if (em()) {
		if (em()->energy() > 0) {
			...
			if ((gap_adjust_time > 0.0) && (status_ == RECV)) {
				em()->DecrTxEnergy(gap_adjust_time, Pt_consume_ - Pr_consume_);
			}
...
int WirelessPhyInterface::sendUp(Packet *p) {
	/*
	 * Sanity Check
	 */
	assert(initialized());
	...
	if (em()) {
		...
		if (isSleeping() == true && (isInterfaceOn() == true)) {
			pkt_recvd = 0;
			goto DONE;
		}
	...
	DONE: p->txinfo_.getAntenna()->release();
...
void WirelessPhyInterface::setInterfaceOn() {
...
void WirelessPhyInterface::setInterfaceOff() {

	isInterfaceOn_ = false;
	status_ = SLEEP;
...
void WirelessPhyInterface::wakeInterfaceUp() {
...
void WirelessPhyInterface::putInterfaceToSleep() {
	if (status_ == SLEEP)
		return;
...
//
void WirelessPhyInterface::dump(void) const {
	Phy::dump();
...
void WirelessPhyInterface::updateIdleEnergy() {
	if (em() == NULL) {
		return;
	}
	if (NOW > update_energy_time_ && (isInterfaceOn() == TRUE && status_ == IDLE)) {
		em()-> DecrIdleEnergy(NOW - update_energy_time_, P_idle_);
		update_energy_time_ = NOW;
	}
...
double WirelessPhyInterface::getDist(double Pr, double Pt, double Gt,
...
//
void WirelessPhyInterface::updateSleepEnergy() {
	if (em() == NULL) {
		return;
	}

	if (NOW > update_energy_time_ && (isInterfaceOn() == true && isSleeping() == true)) {
		em()-> DecrSleepEnergy(NOW - update_energy_time_, P_sleep_);
		update_energy_time_ = NOW;
		// log node energy
		if (em()->energy() > 0) {
