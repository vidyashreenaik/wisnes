...
/*
 * Force timers to expire on slottime boundaries.
 */
...
void BackoffTimer::pause() {
	...
	//the calculation below make validation pass for linux though it
	// looks dummy
...
//
void DirectionalNavTimer::start(double time) {
	Scheduler &s = Scheduler::instance();
	assert(busy_ == 0);
...
void DirectionalNavTimer::stop(void) {
	Scheduler &s = Scheduler::instance();
...
void DirectionalNavTimer::handle(Event *) {
	busy_ = 0;
	stime = 0.0;
	rtime = 0.0;

	directionalNav_->timerHandler();
}
...
/* ======================================================================
 DirectionalNav Class Functions
 ====================================================================== */
...
bool DirectionalNav::isCovering(double direction) const {
	double halfWidth = width_ / 2;

	double lowerBound = direction_ - halfWidth;
	if (lowerBound < 0)
		lowerBound = lowerBound + 360;

	double upperBound = direction_ + halfWidth;
	if (upperBound >= 360)
	...
	if (upperBound >= lowerBound) {
		...
		if ((direction >= lowerBound && direction < 360) || (direction >= 0
...
void DirectionalNav::timerHandler() {
}
...
Mac802_11DVCS::Mac802_11DVCS() :
	Mac(), phymib_(this), macmib_(this), mhIF_(this), mhNav_(this), mhRecv_(
			this), mhSend_(this), mhDefer_(this), mhBackoff_(this), mhBeacon_(
			this), mhProbe_(this) {
	...
	// Check if basic/data rates are set otherwise use bandwidth_ as default
...
inline int Mac802_11DVCS::is_idle() {
	...
	//TODO: Check all directional NAVs to see if they're still valid
	...
	while (iter != directionalNavs_.end()) {
		...
		if (directionalNav->getExpirationTime() > Scheduler::instance().clock()) {
			// return false
		}
		++iter;
	}
...
void Mac802_11DVCS::capture(Packet *p) {
	...
	// Check direction of the destination node
	double direction = 0.0; //TODO: Get direction
	...
	if (iter == directionalNavs_.end()) {
		DirectionalNav* directionalNav = new DirectionalNav(60, direction,
				usec(phymib_.getEIFS() + txtime(p)));
		directionalNavs_[direction] = directionalNav;
	} else {
		(*iter).second->setExpirationTime(usec(phymib_.getEIFS() + txtime(p)));
	}
...
int Mac802_11DVCS::check_pktTx() {
	...
	case MAC_Subtype_Data:
		...
		if ((u_int32_t) ETHER_ADDR(mh->dh_ra) != MAC_BROADCAST)
			timeout = txtime(pktTx_) + DSSS_MaxPropagationDelay
...
void Mac802_11DVCS::recv_timer() {
	...
	if (rx_state_ == MAC_COLL) {
		discard(pktRx_, DROP_MAC_COLLISION);
		//TODO: Update the direction NAV based on destination location
	...
	if (tap_ && type == MAC_Type_Data && MAC_Subtype_Data == subtype)
		tap_->tap(pktRx_);
	/*
	 * Adaptive Fidelity Algorithm Support - neighbourhood information
	 * collection
	 *
	 * Hacking: Before filter the packet, log the neighbour node
	 * I can hear the packet, the source is my neighbour
	 */
